...
| "готов(а) начать"            | Resets data, changes state = "waiting", sends part-of-speech selection                     |
| "давай отработаем ошибки"    | If no mistakes, "Ошибок нет.", state = "waiting". If mistakes exist, state = "repeat", shuffles mistake list, sets current_word via pop() |
| "хочу выбрать другую часть речи" | Allows user to change the part of speech                                                 |
